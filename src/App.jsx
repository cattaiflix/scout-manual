...
const emptyPeriod = () => ({
  formacao: "", ataque: "", defesa: "", saida: "", destaques: "",
  gols1: [], gols2: [],
});
const emptyGoal = () => ({ tipo: "rolando", obs: "" });
const emptyPenalty = () => ({ jogador: "", camisa: "", zona: "", resultado: "gol", tipo: "batedor" });
  ...
  const teamBlock = (teamKey, teamName) => (
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-bold text-[#7fb3f5] truncate">{teamName || teamKey === "gols1" ? (teamName || "Time 1") : (teamName || "Time 2")}</span>
    ...
    <div className="bg-[#060e1a] rounded-lg p-3 border border-[#1e3a5f] mb-3">
      ...
      <div className="flex gap-3">
        ...
        <div className="flex flex-col items-center justify-start pt-5">
          <span className="text-[#4a6fa5] font-bold">x</span>
        </div>
...
export default function App() {
  ...
  const penTotal1 = penalties.filter(p => p.tipo === "batedor" && p.resultado === "gol").length;
  const penTotal2 = penalties.filter(p => p.tipo === "goleiro" && p.resultado === "gol").length;
        ...
        <div className="max-w-3xl mx-auto p-4 space-y-4 no-print">
          ...
          <div className={cc}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              {[["time1","logo1","Time 1 (Casa)"],["time2","logo2","Time 2 (Visitante)"]].map(([tk,lk,lbl]) => (
                <div key={tk}>
            ...
            <div className="mt-3 bg-[#060e1a] rounded-lg p-3 border border-[#1e3a5f]">
              <p className="text-xs text-[#4a6fa5] uppercase font-semibold mb-1">Placar Total (calculado)</p>
              <div className="flex items-center gap-3">
                ...
                {(penTotal1 > 0 || penTotal2 > 0) && (
                  <span className="ml-2 text-xs text-yellow-400">(Pen: {penTotal1} x {penTotal2})</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className={cc}>
            ...
            {periods.map((p, i) => activeTab === i && (
              <div key={i} className="space-y-3">
                ...
                <PeriodScore
                  period={p}
                  onChange={upd => updatePeriod(i, upd)}
                  time1={header.time1}
                  time2={header.time2}
                />
                ...
                <div>
                  ...
                  <VoiceTextarea className={ic} placeholder="Nome, camisa e características dos destaques..." value={p.destaques} onChange={v => updatePeriod(i, { destaques: v })} />
                </div>
              </div>
            ))}
          </div>
          ...
          {/* PÊNALTIS */}
          <div className={cc}>
            <h2 className="text-sm font-bold text-[#7fb3f5] uppercase tracking-widest mb-3">🥅 Cobranças de Pênalti</h2>

            {/* Goleiro */}
            <div className="bg-[#0d1b2a] rounded-lg p-3 mb-4 border border-[#1e3a5f]">
              <h3 className="text-xs font-bold text-[#f59e0b] uppercase mb-2">🧤 Goleiro Adversário</h3>
            ...
            <div className="bg-[#0d1b2a] rounded-lg p-3 mb-3 border border-[#1e3a5f]">
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div>
                  ...
                  <div className="flex gap-2">
                    {[["batedor","⚽ Batedor"],["goleiro","🧤 Goleiro"]].map(([v,l]) => (
                      <button key={v} type="button" onClick={() => setNewPen(n => ({ ...n, tipo: v }))}
                        className={`flex-1 py-1.5 rounded-lg text-xs font-medium border transition-colors ${newPen.tipo === v ? "bg-[#3b82f6] border-[#3b82f6] text-white" : "border-[#1e3a5f] text-[#4a6fa5] hover:text-white"}`}>
                        {l}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    {[["gol","✅ Gol"],["defendido","🛑 Def."],["fora","❌ Fora"]].map(([v,l]) => (
                      <button key={v} type="button" onClick={() => setNewPen(n => ({ ...n, resultado: v }))}
                        className={`flex-1 py-1 rounded text-xs font-medium border transition-colors ${newPen.resultado === v ? "bg-[#3b82f6] border-[#3b82f6] text-white" : "border-[#1e3a5f] text-[#4a6fa5] hover:text-white"}`}>
                        {l}
                      </button>
                    ))}
                  </div>
                ...
                {penalties.map((pen, idx) => (
                  <div key={pen.id} className="flex items-center justify-between bg-[#0d1b2a] rounded-lg px-3 py-2 border border-[#1e3a5f]">
                    <div className="flex items-center gap-2 text-sm flex-wrap">
                      ...
                      <span className={`text-xs ${pen.resultado === "gol" ? "text-green-400" : pen.resultado === "defendido" ? "text-red-400" : "text-yellow-400"}`}>
                        {pen.resultado === "gol" ? "✅" : pen.resultado === "defendido" ? "🛑" : "❌"}
                      </span>
                    </div>
        ...
        <div className="max-w-3xl mx-auto p-4 space-y-4">
          ...
          <div className="bg-[#0a1628] border border-[#1e3a5f] rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-3xl font-black text-[#3b82f6]">{total1}</p>
                </div>
              </div>
              <div className="text-center">
                ...
                {(penTotal1 > 0 || penTotal2 > 0) && (
                  <p className="text-xs text-yellow-400 mt-1 font-semibold">Pen: {penTotal1} x {penTotal2}</p>
                )}
              </div>
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <p className="text-lg font-bold">{header.time2 || "Time 2"}</p>
                  <p className="text-3xl font-black text-[#3b82f6]">{total2}</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-2 border-t border-[#1e3a5f] pt-3 mb-3">
              {periods.map((p, i) => (
                <div key={i} className="text-center bg-[#060e1a] rounded-lg py-2 px-1">
            ...
            {header.observador && (
              <div className="border-t border-[#1e3a5f] pt-3 flex items-center gap-2">
                <span className="text-[#4a6fa5] text-xs uppercase font-semibold">👤 Observador:</span>
                <span className="text-white text-sm font-medium">{header.observador}</span>
              </div>
            )}
          </div>
            ...
            <div key={i} className="bg-[#0a1628] border border-[#1e3a5f] rounded-xl p-4">
              ...
              {(p.gols1.length > 0 || p.gols2.length > 0) && (
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {[["gols1", header.time1 || "Time 1"], ["gols2", header.time2 || "Time 2"]].map(([key, name]) => (
                    p[key].length > 0 && (
                      <div key={key} className="bg-[#0d1b2a] rounded-lg p-2 border border-[#1e3a5f]">
              ...
              <div className="grid grid-cols-1 gap-2 text-sm">
                {[["⚔️ Ataque",p.ataque],["🛡 Defesa",p.defesa],["🔄 Saída",p.saida],["⭐ Destaques",p.destaques]].map(([l,v]) => v && (
                  <div key={l} className="bg-[#0d1b2a] rounded-lg p-3 border border-[#1e3a5f]">
          ...
          {(penalties.length > 0 || goalie.observacoes) && (
            <div className="bg-[#0a1628] border border-[#1e3a5f] rounded-xl p-4">
              <h3 className="font-bold mb-3">🥅 Análise de Pênaltis</h3>
              <div className="bg-[#0d1b2a] rounded-lg p-3 border border-[#1e3a5f] mb-3">
                <p className="text-xs font-bold text-[#f59e0b] uppercase mb-1">🧤 Goleiro Adversário</p>
              ...
              {penalties.length > 0 && (
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {[["⚽ Onde bateram", bateStats], ["🧤 Onde o goleiro pulou", golStats]].map(([title, stats]) => (
                    <div key={title} className="bg-[#0d1b2a] rounded-lg p-3 border border-[#1e3a5f]">
                      ...
                      <div className="border-2 border-[#1e3a5f] rounded overflow-hidden">
                        <div className="grid grid-cols-3">
                          {ZONES.map(z => {
                            ...
                            const bg = count === 0 ? "bg-[#0a1628]" : intensity > 0.7 ? "bg-[#ef4444]" : intensity > 0.4 ? "bg-[#f59e0b]" : "bg-[#22c55e]";
                            return (
                        ...
                        <div className="text-center bg-[#1e3a5f] py-1">
              ...
              {batedores.length > 0 && (
                <div className="bg-[#0d1b2a] rounded-lg p-3 border border-[#1e3a5f]">
                  <p className="text-xs font-bold text-[#a78bfa] uppercase mb-2">Lista de Batedores</p>
                  <div className="space-y-1">
                    {batedores.map((pen, idx) => (
                      <div key={pen.id} className="flex items-center gap-2 text-sm">
                        <span className="text-[#4a6fa5] text-xs w-4">{idx+1}.</span>
                        <span className="font-medium">{pen.jogador}</span>
                        {pen.camisa && <span className="text-[#4a6fa5] text-xs">(#{pen.camisa})</span>}
                        <span className="text-[#7fb3f5] text-xs">→ {ZONE_LABELS[pen.zona]}</span>
                        <span className={`ml-auto text-xs ${pen.resultado === "gol" ? "text-green-400" : pen.resultado === "defendido" ? "text-red-400" : "text-yellow-400"}`}>
                          {pen.resultado === "gol" ? "✅ Gol" : pen.resultado === "defendido" ? "🛑 Defendido" : "❌ Fora"}
                        </span>
                      </div>
                    ))}
                  </div>
